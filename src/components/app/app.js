...
export default class App extends Component {
  ...
  shortText = (text, index) => {
    let shortenedText = text.slice(0, index)
    let lastLetter = index - 1
    while (shortenedText[lastLetter] !== ' ') {
      shortenedText = shortenedText.slice(0, lastLetter)
      lastLetter--
    }
    shortenedText += ' ...'
  ...
  shortTitle = (title) => {
    title = title.length > 26 ? this.shortText(title, 26) : title
    return title
  }

  shortOverview = (text, title) => {
    if (title.length > 13 && text.length > 160) {
      text = this.shortText(text, 160)
    }
    if (title.length <= 13 && text.length > 200) {
      text = this.shortText(text, 200)
    }
    return text
  }
    ...
    this.getResource
      ...
      .then((movies) => {
        if (this.state.activeKey == 1) {
          ...
          if (localStorage.length < this.state.currentPage * 20) {
            this.setState({ totalResults: this.state.currentPage * 20 })
          } else {
          ...
          this.setState(
            ({ movieData }) => {
              ...
              const newItem = {
                ...
                overview: this.shortOverview(movie.overview, movie.title),
                userRate: rate,
              }
